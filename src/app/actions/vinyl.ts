...
import { createVinyl, deleteVinyl, getVinyl, listVinyls, updateVinyl } from "@/lib/store"
...
export async function getVinylsAction(sessionId: string): Promise<ActionResult<Vinyl[]>> {
...
export async function getVinylAction(sessionId: string, id: string): Promise<ActionResult<Vinyl>> {
	if (!isValidSessionId(sessionId)) {
		return invalidSessionResult()
	}
...
export async function createVinylAction(sessionId: string, input: unknown): Promise<ActionResult<Vinyl>> {
	if (!isValidSessionId(sessionId)) {
		return invalidSessionResult()
	}
